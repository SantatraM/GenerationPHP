...
public class TraitementDonneeBase {
    ...
    public void GenerateControlleur(Table table,String path) throws Exception {
        ...
        for(int j=1;j<table.getColonnes().length;j++){
            inputvalue = inputvalue + "$"+table.getColonnes()[j].getNom()+"=$this->input->post('"+table.getColonnes()[j].getNom()+"'); \n";
            colonne = colonne + table.getColonnes()[j].getNom();
            if(table.getColonnes()[j].getType().equals("integer") || table.getColonnes()[j].getType().equals("double precision") || table.getColonnes()[j].getType().equals("decimal")) {
        ...
        values += ".\")\"";
        nouveau += ".\")\"";
        for(int i=0;i<table.getColonnes().length;i++){
    ...
    public String creationChampColonneUpdate(Table t,Colonne c) throws Exception{
        if(c.isPrimary() == true){
            ...
            return "       <input name='" + c.getNom() + "' type='" + g.getRemplacement().get(c.getType()) + "' value='<?php echo $"+t.getNom()+"->"+c.getNom()+"; ?>' hidden>";
        }else if (c.isForeign() == false) {
            Generateur g = new Generateur();
            return "       <input name='" + c.getNom() + "' type='" + g.getRemplacement().get(c.getType()) + "' value='<?php echo $"+t.getNom()+"->"+c.getNom()+"; ?>'>";
        } else {
            String option = "";
